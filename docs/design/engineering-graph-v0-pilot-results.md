...
## Multi-consumer and multi-question stress check

A shared graph was exercised with independent `IMPLEMENTATION` and
`RELEASE-AUDIT` consumers plus parallel architecture/interface production.

Observed behavior:

- each Consumer derives only its own recursive upstream closure; another
  Consumer does not pollute the target;
- two simultaneously actionable branches are both CREATE;
- independent Questions block only their affected capabilities;
- WAIT on one branch does not suppress CREATE on another branch;
- a downstream capability remains PENDING until all of its production
  prerequisites are satisfied;
- if no CREATE remains and unresolved Questions block the frontier, the target
  is BLOCKED.

No aggregate multi-target runtime entity was needed. A Consumer remains a
selection boundary over the same normative graph. If a caller needs several
targets, independent evaluations preserve each target's meaning and may be
combined by the caller as an execution/reporting view.

This check strengthens the interpretation that Questions are dynamic semantic
feedback edges while stable production topology can remain a DAG.


## Upstream feedback and repair

A downstream-discovered contradiction was modeled as a Question addressed to the
upstream PRODUCT Authority while accepted Requirements and Architecture providers
already existed.

Observed behavior:

- the unresolved Question blocks the accepted upstream Requirements provider;
- dependency propagation makes downstream Architecture unavailable without
  introducing a production cycle;
- the target becomes BLOCKED;
- an accepted PRODUCT-owned repair artifact can resolve the Question;
- reevaluation then restores the same target to COMPLETE.

This is evidence that semantic feedback does not require static cyclic production
dependencies. Stable production topology remains a DAG; feedback is represented
by Questions plus canonical upstream repair and reevaluation.


## Second applicability shape

A persistence applicability case was tested independently of the NAPMS
asynchronous-messaging rule.

The same accepted deployment topology supports two project policy projections:

- a stateless implementation consumes accepted
  `deployment.persistence-not-required` evidence and becomes COMPLETE without
  creating a DATA design;
- a stateful implementation consumes `deployment.persistence-design` and
  exposes that capability as CREATE when it is missing.

No Harness-level N/A state or conditional edge was required. This provides a
second materially different example supporting the rule that applicability is
project-owned policy projected into ordinary capability requirements.


## knowledge_kind boundary review

The routing experiments now provide enough evidence to classify
`knowledge_kind`.

It is semantic metadata of a production contract, not a skill identifier and not
Core state:

- CapabilityId answers *which project-specific accepted knowledge is needed*;
- Authority answers *who owns the decision*;
- prerequisites answer *which accepted knowledge is needed to form it*;
- knowledge_kind answers *what reusable kind of knowledge is being formed*;
- the external registry independently maps that kind to an installed skill.
...
Decision for v0: retain optional `knowledge_kind` in production contracts and
keep skill paths exclusively in the execution-layer registry.


## Engineering Graph v0 research review

Current evidence set:

- Harness acceptance fixtures;
- complete greenfield CSV Deduplicator progression;
- Nutrition Management regression against the current Harness research head;
- NAPMS regression and adapter projection against the current Harness research head;
- multi-consumer/multi-question stress case;
- upstream semantic feedback and repair;
- two materially different applicability-evidence cases;
- complete greenfield CREATE-to-skill routing coverage.

Review findings:

- **P0:** none known.
- **P1:** none known.
- **P2:** first-class parameterized/scoped capabilities remain intentionally
  deferred; current explicit scoped CapabilityIds are correct but less elegant.
- **P2:** the reference-catalog term `baseline` may be misread as mandatory even
  though catalog semantics explicitly say it is not; this is nomenclature debt,
  not a runtime-model defect.
- **P3:** aggregate reporting across several Consumers is not a Core concern;
  independent target evaluation is sufficient until a real caller demonstrates
  additional semantics.

No current evidence requires Stage, Gate, Task, N/A, MultiTarget, static feedback
cycles, or another Core entity.

Research status: the fundamental Engineering Graph v0 model is no longer blocked
by a known architectural gap. It should remain experimental until the current
head is re-pinned and re-run by the real Nutrition and NAPMS pilots after this
final research sequence.
